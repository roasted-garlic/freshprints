...
describe("printRequestQuotaUserCopy", () => {
  it("show customer limit message is spots-exhausted copy", () => {
    ...
    assert.equal(
      msg,
      "You've used all 25 print spots on this show. Choose another show for more designs.",
    );
  ...
  it("defaults to 25 when cap is missing", () => {
    assert.match(formatShowCustomerLimitUserMessage(null), /all 25 print spots/);
    assert.match(formatShowCustomerLimitUserMessage(undefined), /all 25 print spots/);
  });
});
