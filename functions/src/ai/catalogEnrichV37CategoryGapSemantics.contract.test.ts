...
describe("catalog-enrich-v37 false category-gap semantics corrective", () => {
  ...
  it("keeps visual-first baseline and defines categoryGapNote as true-gap only", () => {
    ...
    assert.match(
      DEFAULT_AI_ENRICHMENT_PROMPT_TEMPLATE,
      /Do not use categoryGapNote to explain or justify a valid category choice/,
    );
    assert.match(
      DEFAULT_AI_ENRICHMENT_PROMPT_TEMPLATE,
      /Use categoryAlternatives only when another approved category is genuinely plausible/,
    );
    assert.doesNotMatch(DEFAULT_AI_ENRICHMENT_PROMPT_TEMPLATE, /cucumber|Funny & Sarcastic|Food & Drink|pin-up|Woman holding/i);
    assert.ok(DEFAULT_AI_ENRICHMENT_PROMPT_TEMPLATE.length < 4500);
  });

  it("auto-upgrades archived v36 default to v37", () => {
