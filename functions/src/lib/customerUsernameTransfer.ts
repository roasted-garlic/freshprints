...
export function buildDuplicateSourcePlaceholderUsername(customerId: string): string {
  ...
  const candidate = `dupe-src-${suffix}`;
  const validated = validateCustomerUsername(candidate);

  if (!validated.isValid) {
    throw new Error("Unable to derive a safe placeholder username for the source account.");
  }

  return validated.username;
}

export interface UsernameTransferTransactionInput {
...
export interface UsernameTransferTransactionResult {
...
export async function applyDuplicateUsernameTransferTransaction(
  input: UsernameTransferTransactionInput,
): Promise<UsernameTransferTransactionResult> {
  ...
  return adminDb.runTransaction(async (transaction) => {
    ...
    const sourcePlaceholderUsername = buildDuplicateSourcePlaceholderUsername(input.sourceCustomerId);
