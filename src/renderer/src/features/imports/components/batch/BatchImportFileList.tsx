...
import { isValidatedFileIncluded, sliceFileListPreview } from "../../utils/batchImportDisplay";
...
export function BatchImportFileList({
  ...
  const { items, remainingCount } = sliceFileListPreview(files);
  ...
  return (
    <div className="batch-import-file-list">
      ...
      <ul>
        {items.map((file) => {
          ...
          const validationWarnings =
            variant === "validated" && file.validation ? file.validation.warnings : [];
          const isIncluded =
            variant !== "validated" ||
            !excludedFilePaths ||
      ...
      {remainingCount > 0 ? (
        <p className="batch-import-file-list-remaining">and {remainingCount} more</p>
      ) : null}
    </div>
  );
}
