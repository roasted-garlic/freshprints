...
import type { FirebaseDebugReport } from '@fresh-prints/shared/utils/firebaseDebugReport';
import {
  formatElapsedMs,
  formatEventAction,
  formatEventRoute,
...
export function FirebaseDebugPanel({
  ...
  const traceEnabled = report.session.status === 'active';
  ...
  const handleCopyReport = async () => {
    await navigator.clipboard.writeText(JSON.stringify(copyPayload ?? report, null, 2));
    ...
    <div style={styles.overlay}>
      <div style={styles.panel}>
        <div style={styles.header}>
          <div>
            <p style={styles.title}>Firebase Debug — Portal</p>
            <p style={styles.subtitle}>
              project: {projectId || '(unknown)'} · route:{' '}
              {report.session.routesVisited.at(-1) ?? '—'} · elapsed:{' '}
              {formatElapsedMs(report.session.elapsedMs)} · tracing:{' '}
        ...
        {report.session.status === 'inactive' ? (
          <p style={styles.disclaimer}>
            Tracing inactive. This is not a completed zero-activity report. Enable tracing before testing.
          </p>
        ) : null}

        <div style={styles.controls}>
          <button onClick={onToggleTracing} style={styles.button} type="button">
            {traceEnabled ? 'Disable tracing' : 'Enable tracing'}
          </button>
          <button onClick={onReset} style={styles.button} type="button">
            Reset
          </button>
