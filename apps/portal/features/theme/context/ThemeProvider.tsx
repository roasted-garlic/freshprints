...
function readResolvedThemeFromDom(): 'light' | 'dark' {
  ...
  return resolveTheme(themeService.getStoredThemeMode());
}
...
export function ThemeProvider({ children }: ThemeProviderProps) {
  const [themeMode, setThemeModeState] = useState<ThemeMode>(() =>
    typeof window === 'undefined' ? 'light' : themeService.getStoredThemeMode(),
  );
  const [resolvedTheme, setResolvedTheme] = useState<'light' | 'dark'>(() =>
    typeof window === 'undefined' ? 'light' : readResolvedThemeFromDom(),
  );
