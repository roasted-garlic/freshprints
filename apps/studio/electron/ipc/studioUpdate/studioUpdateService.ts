...
function errorMessageFrom(error: unknown, fallback: string): string {
  // Never surface the raw error object — it may include response headers/auth details from the
  // update feed request. Only a plain message string is safe to show or log.
...
async function getAutoUpdater(): Promise<AutoUpdaterLike | null> {
  ...
  updater.channel = resolveStudioUpdateChannel();
  ...
  updater.on("error", (error) => {
    setState(applyUpdateError(state, errorMessageFrom(error, "The update check failed.")));
  });
...
export async function downloadStudioUpdate(): Promise<StudioUpdateState> {
  ...
  try {
    ...
    setState(applyUpdateError(state, errorMessageFrom(error, "The update download failed.")));
  } finally {
