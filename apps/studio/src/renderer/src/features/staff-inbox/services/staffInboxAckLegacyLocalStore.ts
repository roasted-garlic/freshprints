...
/** Legacy localStorage shape — used only for one-time migration to Firestore. */
...
export function loadLegacyStaffInboxAckRecords(userId: string): StaffInboxAckRecord[] {
  if (!userId.trim()) {
    return [];
  }
...
export function clearLegacyStaffInboxAckLocalStorage(userId: string): void {
  if (!userId.trim()) {
    return;
  }
