 ...
 * "correct" than another) — and its persisted `isInternal` matches the selected Customer/Internal
 * list. Without these guards, a request fetched while one tab or kind was active can be merged in
 * after the hook has since switched, permanently contaminating the other list until the next full
 * reload. Mirrors the identical, already-reviewed guard in `mergeShowQueuePrintRequestSources`.
 */
export function mergePrintRequestsById(
  ...
  for (const request of additions) {
    if (request.isInternal !== isInternal && typeof isInternal === "boolean") {
      continue;
    }
