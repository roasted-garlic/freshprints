...
export function resolveArtworkEnhanceCallableErrorMessage(error: unknown): string {
  if (error instanceof FirebaseError) {
    ...
    switch (error.code) {
      case "functions/unauthenticated":
        return "You must be signed in to change upscale settings.";
      case "functions/permission-denied":
        return message && !isGenericCallableMessage(message)
          ? message
          : "You do not have permission to change upscale settings.";
      case "functions/invalid-argument":
      case "functions/failed-precondition":
        return message && !isGenericCallableMessage(message)
          ? message
          : "Upscale could not be updated for this item.";
      case "functions/unavailable":
      case "functions/not-found":
      case "functions/internal":
        return "Upscale is unavailable right now. Confirm Cloud Functions are deployed for this Firebase project.";
  ...
  return "Unable to update upscale setting.";
}
