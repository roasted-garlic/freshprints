...
describe("derivePrintRequestsListLoading", () => {
  it("reports loading on initial mount, before any tab has ever completed a load", () => {
    assert.equal(derivePrintRequestsListLoading(true, null, "working", null, "customer"), true);
  });

  it("reports not loading once state.isLoading is false and the loaded tab and kind match", () => {
    assert.equal(
      derivePrintRequestsListLoading(false, "working", "working", "customer", "customer"),
      false,
    );
  });
  ...
  it("reports loading during the transitional render: activeTab changed but state.isLoading has not yet flipped true (Queued -> Working stale-list regression)", () => {
    ...
    assert.equal(
      derivePrintRequestsListLoading(false, "queued", "working", "customer", "customer"),
      true,
    );
  });

  it("reports loading during the transitional render when kind changes (Customer -> Internal)", () => {
    assert.equal(
      derivePrintRequestsListLoading(false, "working", "working", "customer", "internal"),
      true,
    );
  });
  ...
  it("reports loading while state.isLoading is genuinely true, regardless of loadedTab", () => {
    assert.equal(
      derivePrintRequestsListLoading(true, "working", "working", "customer", "customer"),
      true,
    );
  });
  ...
  it("reports loading for every tab during rapid switching until the last-requested tab's load completes", () => {
    ...
    assert.equal(
      derivePrintRequestsListLoading(false, "queued", "printing", "customer", "customer"),
      true,
    );
    assert.equal(
      derivePrintRequestsListLoading(false, "queued", "working", "customer", "customer"),
