...
function isAiProcessingStage(value: unknown): value is AiProcessingStage {
  return (
    typeof value === "string" &&
    AI_PROCESSING_STAGES.includes(value as AiProcessingStage)
  );
}

function mapVisibleTextColor(
  value: unknown,
): DesignAiAnalysis["visibleTextColor"] {
  if (
    value === "black" ||
    value === "white" ||
    value === "mixed" ||
...
function mapAiSuggestions(value: unknown): DesignAiSuggestions | undefined {
  ...
  return {
    ...
    promptTokens:
      typeof data.promptTokens === "number" ? data.promptTokens : null,
    completionTokens:
      typeof data.completionTokens === "number" ? data.completionTokens : null,
    estimatedCostUsd:
...
function mapAiAnalysis(value: unknown): DesignAiAnalysis | undefined {
  ...
  return {
    primarySubject:
      typeof data.primarySubject === "string" ? data.primarySubject : undefined,
    secondarySubjects: Array.isArray(data.secondarySubjects)
      ? data.secondarySubjects.filter(
    ...
    colorPalette: Array.isArray(data.colorPalette)
      ...
      : undefined,
    artworkContainsText:
      typeof data.artworkContainsText === "boolean"
        ? data.artworkContainsText
        : undefined,
    visibleText: Array.isArray(data.visibleText)
      ? data.visibleText.filter(
          (item): item is string => typeof item === "string",
        )
      : undefined,
    visibleTextColor: mapVisibleTextColor(data.visibleTextColor),
    textRecognitionConfidence:
      typeof data.textRecognitionConfidence === "number"
        ? data.textRecognitionConfidence
        : undefined,
    spellingConfidence:
      typeof data.spellingConfidence === "number"
        ? data.spellingConfidence
        : undefined,
    transparencyConfidence:
      typeof data.transparencyConfidence === "number"
        ? data.transparencyConfidence
        : undefined,
    estimatedPrintComplexity:
      typeof data.estimatedPrintComplexity === "string"
        ? data.estimatedPrintComplexity
        : undefined,
    trademarkWarning:
      typeof data.trademarkWarning === "string"
        ? data.trademarkWarning
        : undefined,
    overallConfidence:
