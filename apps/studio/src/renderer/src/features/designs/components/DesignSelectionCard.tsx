...
interface DesignSelectionCardProps {
  ...
  /** Opens parent-owned lightbox — must not mutate selection membership/qty. */
...
export function DesignSelectionCard({
  ...
  // `isExistingSelection` remains part of the public props contract for callers.
    ...
    <div className={`design-selection-card${isSelected ? " is-selected" : ""}`}>
      <div className="design-selection-card-image-wrap">
        <DesignThumbnailPanel
          ...
          imageFit="contain"
          interactive={Boolean(onOpenPreview)}
          loadingLabel="Loading thumbnail"
          onImageClick={onOpenPreview ? () => onOpenPreview(design) : undefined}
        />
