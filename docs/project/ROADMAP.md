...
| 13 | `production-release` — prod Firebase / App Hosting / Google / email | **Active** — Class D closed; Stage 1C fixture recorded (`s9Yi7i8uq2ZddERyDuNT`); Stage 1B show pending; Stage 2 not started. Brand mapping approved; brand implement before Stage 2. |
...
**Active managed goal:** `production-release` (Goal #13) — Storage Class D **closed**; Stage 1C
catalog fixture **recorded** (QA design reused). Next: Stage 1B `Production Smoke Test Show` →
confirm Stage 1 fixtures complete → bundled-brand implement (gated) **before** Stage 2
hosted.app smoke → Stage 3 gate → Stage 4 domain. See Stage 1 checkpoint.
