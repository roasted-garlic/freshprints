...
| 13 | `production-release` — prod Firebase / App Hosting / Google / email | **Active — BLOCKED on Portal App Hosting rollout** of registration loading-state fix. Client fix on `development`. Stage 1 complete; Class D closed; brand/Stage 2 paused. |
...
**Active managed goal:** `production-release` (Goal #13) — Portal registration loading-state fix
implemented on `development`; await `APPROVE PRODUCTION PORTAL APP HOSTING ROLLOUT`. Stage 1
complete; Class D closed; brand + Stage 2 paused.
