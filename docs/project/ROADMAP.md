...
> 2026-08-13: **Studio 1.0.4 P4 PUBLISHED + repository consolidation closeout (in progress)** — Release **370305556** / tag `v1.0.4-e59205d` published; prod tip **`e59205d7`** (PR #70); Firebase `firestore:rules` + `deleteEligibleUnapprovedDesign` **DEPLOYED**; dual-platform smoke **PASS**; fixtures **already gone** (verified). Historical draft **369614747** anomaly — do not restore. Local branch `closeout/prod-into-development` merges production lineage into development (**push pending**). **Phase 9 parked.** Domain cutover still gated (`APPROVE MYPRINTREQUEST.COM CUTOVER`). PRODUCTION_BEFORE must remain `e59205d7`.
>
