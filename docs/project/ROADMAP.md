...
> 2026-08-14: **Studio Design Library archive/restore/companion — DEV Signoff** — Goal `studio-design-library-archive-restore-reconciliation`. Owner QA **PASS** (A/B/C/D+D1/D2). DEV Rules+indexes on `fresh-prints-dev`. Signoff **approved_with_notes**. Production promotion gated (PR → Rules/indexes only → Studio release → smoke). Phrase: `APPROVE PROD FIRESTORE RULES AND INDEXES DEPLOY FOR DESIGN LIBRARY CORRECTIVE`. Signoff: `docs/workflow/reviews/2026-08-14-studio-design-library-archive-restore-reconciliation-signoff.md`.
>
