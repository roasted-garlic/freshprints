...
> 2026-08-17: **Portal GA4 event transmission corrective — production candidate** — Goal `portal-ga4-event-transmission-corrective`. Bootstrap `gtag('js', new Date())` in the Portal GA stub. Implement/Test Signoff **approved**; owner **DEV transport QA: PASS** (`g/collect` proven on a local TEST stream). **Production PR not yet authorized.** Live production remains `124c6fa` / `fresh-prints-portal-build-2026-08-17-002` (tag present; pre-fix collect was zero). Production QA after rollout must prove `g/collect`, not tag detection. Signoff: `docs/workflow/reviews/2026-08-17-portal-ga4-event-transmission-corrective-signoff.md`.
>
