...
> 2026-08-18: **Portal Add to Show Unmissable — SIGNOFF APPROVED (DEV)** — Goal `portal-add-to-show-unmissable` **DONE**. Copy/presentation only: drawer **Review & Add to Show** + helper + **Needs a show**; review header drops competing Upload/Browse; CTA **Add Request to Whatnot Show** (opens picker; wider desktop / full-width mobile). No backend/lifecycle change (ADR-FP-066). Owner `DEV ADD TO SHOW UNMISSABLE QA: PASS`. Signoff: `docs/workflow/reviews/2026-08-18-portal-add-to-show-unmissable-signoff.md`. Production PR later. Analytics (`portal-design-engagement-analytics`) remains queued separately.
>
