...
2. **Processing tab queue controls** — **Auto advance** (Studio-local `localStorage`, retaining key `fresh-prints.ai-processing.auto-advance`): **Start AI** / **Pause AI** runs sequential queue; OFF shows **Process image with AI** for one-at-a-time manual stepping. **Default Auto advance = ON** when unset. A legacy sessionStorage value is migrated on first read when no localStorage value exists. This remains separate from the shell-header Auto process preference.
