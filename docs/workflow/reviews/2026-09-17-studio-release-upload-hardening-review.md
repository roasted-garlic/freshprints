...
- The proposed change remains limited to the existing finalizer and its focused contract tests. It preserves the current production reachability, exact SHA, exact release ID, draft-only, canonical naming, dual-platform, and non-stable validation gates.
