...
- The proposed change remains limited to the existing finalizer and its focused contract tests. The retry logic stays inline because the finalizer checks out the exact production build SHA, so no helper from the workflow branch is required at runtime. It preserves the current production reachability, exact SHA, exact release ID, draft-only, canonical naming, dual-platform, and non-stable validation gates.
