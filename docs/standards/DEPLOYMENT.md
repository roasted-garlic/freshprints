...
- **Updater:** One merged `latest-mac.yml` lists both arches' ZIP/DMG URLs. `electron-updater` selects the matching arch by filename. Windows continues to use `latest.yml`. For this internal **ad-hoc** Mac path, automatic update apply is **unsupported / best-effort** until Developer ID + notarization (update check must not crash).
- **Signing:** Mac 1.0.4 internal distribution uses **ad-hoc** codesign (`mac.identity: "-"`, `hardenedRuntime: false`) so the `.app` is structurally valid (sealed `CodeResources`). This is **not** Developer ID and **not** notarized. Workflow selector remains `distribution_mode: internal-unsigned` (no Apple credentials). Selecting `signed` on a stable Mac run fails until a future Apple credential phase. CI runs `codesign -v -vvv --strict --deep` on arm64 and x64 after packaging (fail closed).
