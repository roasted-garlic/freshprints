...
- **Updater:** One merged `latest-mac.yml` lists both arches' ZIP/DMG URLs. `electron-updater` selects the matching arch by filename. Windows continues to use `latest.yml`. For the current **ad-hoc** Mac path (through Studio **1.0.6** packaging until Workstream A2), automatic update **install** remains unsupported — download may succeed but Squirrel.Mac signature validation fails. Install-phase failures now map to a safe install-oriented Settings message (not “check failed”). Developer ID Application signing (`MAC_CSC_LINK` + `MAC_CSC_KEY_PASSWORD`) is the gated fix; notarization is deferred unless those Apple notarization credentials are ready at the same checkpoint.
- **Signing:** Mac packages remain **ad-hoc** (`mac.identity: "-"`, `hardenedRuntime: false`) until the Apple Developer ID credential checkpoint. This is **not** Developer ID and **not** notarized. Stable Mac still requires `distribution_mode: internal-unsigned`. Selecting `signed` for Mac fails closed until `MAC_CSC_*` secrets exist and A2 lands. CI runs `codesign -v -vvv --strict --deep` on arm64 and x64 after packaging (fail closed; currently asserts ad-hoc).
- **Gatekeeper:** Ad-hoc builds are expected to show as **unverified** (Open Anyway / right-click Open). A “damaged / Move to Trash” dialog indicates invalid packaging — do **not** treat clearing quarantine as the product fix. See `docs/workflow/reviews/2026-08-12-studio-1.0.4-macos-smoke-checklist.md`.
- **1.0.6 note:** App version pin is `1.0.6`. Workstream A2 (Developer ID packaging) is intentionally **not** half-converted in CI — prerelease/validation stays runnable without Apple secrets.
