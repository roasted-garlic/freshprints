...
The active customer Continuable invariant is origin-neutral: a non-internal `draft` or `editing`
request is active when it is not parked, whether `requestOrigin` is `studio_customer` or
`portal_customer`. Portal reuses an eligible Studio-created Working/draft and targets that same
request for content mutations. The Studio creation boundary is the transactional
`createStudioCustomerPrintRequest` callable. Parked drafts remain visible history/continuation
candidates but are excluded from active uniqueness and mutation eligibility; empty conflicts are
archived and meaningful conflicts are parked. Existing `customerId` + `status` indexing is reused;
there is no merge, migration, or backfill.
...
**Artwork background vs halftone:** `artworkBackgroundHex` / `artworkBackgroundSource` are display mats only. Dark mat (`#2c2d2d`) does **not** imply or set halftone. Studio human-review toggles may seed the light-black mat when Halftone is enabled and restore the default mat when disabled; a subsequent explicit background choice remains independent and does not change the Halftone decision. Import precedence: explicit background override → all-halftone dark default → code-auto detector → default light (omit field).
...
export interface ShowAllocation {
  ...
  /** Immutable pricing commitment captured when this allocation was created (ADR-FP-185). */
  pricingSnapshot?: {
    policyVersion: string;
    widthTier: "pocket" | "standard_full" | "standard_oversized" | "extra_oversized";
    basePriceUsd: number;
    lengthTier: "standard_length" | "long" | "extra_long" | "extended";
...
interface ShowQueueSettings {
  ...
  // Canonical global Gang Sheet Settings width price/weight fields. Breakpoints are fixed policy.
  ...
  // Canonical height-based length surcharge fields. Bands are fixed policy.
...
The normalized pricing policy uses width boundaries `<=4`, `<=11`, and `<=14` inches for the base
tier, with Pocket requiring both width and height `<=4`. Height surcharge bands are Standard `<=14`,
Long `>14..<=18`, Extra Long `>18..<=24`, and Extended `>24`. Width defaults are
`$1/0.40oz`, `$2/0.75oz`, `$3/0.75oz`, and `$4/0.75oz`; length surcharge defaults are
`$0/$1/$2/$3`. Canonical fields win individually over legacy fallback fields; no migration or
backfill is performed. New allocations capture the resolved price in `pricingSnapshot`.
